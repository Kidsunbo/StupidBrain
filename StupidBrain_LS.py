...
class StupidBrain:
    ...
    class _Util:
        ...
        @classmethod
        def remove_background(cls, background_img_full, seg_img_full, scale, *, need_output=False,
                              output_folder="./output", to_flatten=False, **kwargs):
            """
    ...
    def prepare_data(self, merge=False, *, questions=("0", "1", "2"), question="0", shuffle=True, use_mask=True,
                     prepare_test_data=False, batch_size=32, test_ratio=0.1, **kwargs):
        """
        ...
        def inner_prepare_data():
            ...
            if prepare_test_data and not self.test_data_prepared:
                ...
                temp_for_choose_test = np.concatenate(
                    (np.array(data_names).reshape(-1, 1), np.array(lables).reshape(-1, 4)), axis=1)
...
if __name__ == '__main__':
    s = StupidBrain(train_data_path="../input/train/train/", test_data_path="../input/test/test/",
                    seg_data_path="../input/seg/seg/", csv_data_path="../input/train/train/annotations.csv")
    s.prepare_data(True, questions=("1", '2'), prepare_test_data=True)
    s.run_model(epochs=10)